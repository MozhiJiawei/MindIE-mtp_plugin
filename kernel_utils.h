...
template <typename T, uint32_t precisionMode>
__aicore__ inline void ComputeRmsNormFastWithBias(const AscendC::LocalTensor<T> &out, const AscendC::LocalTensor<float> &in,
    float rms, const AscendC::LocalTensor<T> &gamma, const AscendC::LocalTensor<T> &bias, uint32_t count,
    const AscendC::LocalTensor<float> &tmp, const AscendC::LocalTensor<float> &fp32_g)
{
    float value = 1.0;
    Duplicate(tmp, rms, count);
...
template <typename T, uint32_t precisionMode>
__aicore__ inline void ComputeRmsNormFastWithBiasF32(const AscendC::LocalTensor<T> &out, const AscendC::LocalTensor<float> &in,
    float rms, const AscendC::LocalTensor<T> &gamma, const AscendC::LocalTensor<T> &bias, uint32_t count,
    const AscendC::LocalTensor<float> &tmp, const AscendC::LocalTensor<float> &fp32_g)
{
    float value = 1.0;
                                        ...
                                        size_t tilingSize, AscendC::TPipe *pipe)
{
    ...
    // printf("RoundTile size = %d", roundTilingSize);
