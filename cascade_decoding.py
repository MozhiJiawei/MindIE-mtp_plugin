...
class MtpPlugin(Plugin):
    ...
    def fill_in_model_result(self, model_inputs, model_kwargs, model_output_wrapper, filling_masks):
        speculative_length = self.num_speculative_tokens + 1
    ...
    def rejection_sampling_verify(self, draft_logits, target_logits, draft_tokens, temperature=1.0):
        ...
        """
        # 应用温度并计算概率分布
        draft_logits = F.softmax(draft_logits, dim=-1)
        ...
        for j in range(num_tokens):
            ...
            alpha = 0.5
            # is_accept = self.speculative_judge(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], draft_token)
            # is_accept = self.cascade_chow(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], alpha)
            # is_accept = self.cascade_diff(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], alpha)
            # is_accept = self.cascade_opt(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], alpha)
            # is_accept = self.cascade_token_v1(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], draft_token, alpha)
            # is_accept = self.cascade_token_v2(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], draft_token, alpha)
            is_accept = self.cascade_token_v3(draft_probs[j], target_probs[j], draft_logits[j], target_logits[j], draft_token, alpha)
            ...
            if is_accept:
                ...
                target_token = torch.argmax(target_probs[j]).item()
    ...
    def speculative_judge(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, draft_token):
        q_x = draft_probs[draft_token].item()
        p_x = target_probs[draft_token].item()
    ...
    def cascade_chow(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, alpha):
        max_q = torch.max(draft_probs_unscaled, dim=0).values
        return max_q >= 1 - alpha


    def cascade_diff(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, alpha):
        max_q = torch.max(draft_probs_unscaled, dim=0).values
        max_p = torch.max(target_probs_unscaled, dim=0).values
        return max_q >= max_p - alpha


    def cascade_opt(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, alpha):
        residual = torch.clamp(target_probs - draft_probs, min=0.0)
        residual_sum = residual.sum()
        max_q = torch.max(draft_probs_unscaled, dim=0).values
        max_p = torch.max(target_probs_unscaled, dim=0).values
        return max_q >= (max_p - residual_sum * alpha)


    def cascade_token_v1(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, draft_token, alpha):
        max_p = torch.max(target_probs_unscaled, dim=0).values
        return draft_probs_unscaled[draft_token] >= max_p - alpha


    def cascade_token_v2(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, draft_token, alpha):
        max_p = torch.max(target_probs_unscaled, dim=0).values
        return target_probs_unscaled[draft_token] >= max_p - alpha


    def cascade_token_v3(self, draft_probs, target_probs, draft_probs_unscaled, target_probs_unscaled, draft_token, alpha):
        max_p = torch.max(target_probs_unscaled, dim=0).values
        return target_probs_unscaled[draft_token] >= max_p * (1 - alpha)
    ...
    def plugin_verify(self, sampling_output, cache_ids, result):
        ...
        for batch in range(input_metadata.batch_size):
            ...
            if use_rejection_sampling:
                ...
                if len(accepted_tokens) == num_accepted and num_accepted < len(next_tokens_uncheck):
                    next_tokens_indices.append(list(range(start_pos, start_pos + len(accepted_tokens) + 1)))
                else:
